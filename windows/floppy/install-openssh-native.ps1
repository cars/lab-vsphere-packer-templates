...
$f = Get-WindowsCapability -Online -Name OpenSSH.Server~~~~0.0.1.0

If ($Error.length > 0 ) {
   $Error | select * 
   $Error | select * | Set-Content C:\temp\SSH_GET_CAP.txt
}

# Install the OpenSSH Server
Write-Output "==> Installing Server"
Add-WindowsCapability -Online -LogLevel WarningsInfo -LogPath C:\temp\addcap.txt -Name OpenSSH.Server~~~~0.0.1.0
